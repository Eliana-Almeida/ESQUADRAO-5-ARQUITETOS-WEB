...
def home():
    import requests
    url='https://example.com'; status=''; resultado=False
    if request.method=='POST':
        url=request.form.get('url')
        try: status=requests.get(url,timeout=5).status_code
        except Exception: status='Erro de conexão'
        resultado=True
    return render_template_string(HTML,url=url,status=status,resultado=resultado)
